...
public class StateView extends View {
     ...
     * @return StateView
     */
    public static StateView inject(@NonNull Activity activity, int skeletonLayout) {
        return inject(activity, skeletonLayout, false);
    }
     ...
     * @return StateView
     */
    public static StateView inject(@NonNull Activity activity, int skeletonLayout, boolean hasActionBar) {
        ViewGroup rootView = (ViewGroup) activity.getWindow().getDecorView().findViewById(android.R.id.content);
        return inject(rootView, skeletonLayout, hasActionBar);
    }
     ...
     * @return StateView
     */
    public static StateView inject(@NonNull ViewGroup parent, int skeletonLayout) {
        return inject(parent, skeletonLayout, false);
    }
     ...
     * @return StateView
     */
    public static StateView inject(@NonNull ViewGroup parent, int skeletonLayout, boolean hasActionBar) {
        ...
        StateView stateView = new StateView(parent.getContext(), skeletonLayout);
        if (screenHeight > 0) {
    ...
    public StateView(Context context, int skeletonLayout) {
        this(context);
        mSkeletonLayout = skeletonLayout;
    }
    ...
    public View showEmpty() {
        if (mEmptyView == null) {
            ...
            if (-1 != mSkeletonLayout) {
                // @hl 新增占位布局
                ViewStub viewStub = mEmptyView.findViewById(R.id.be_stateview_bgVs);
                viewStub.setLayoutResource(mSkeletonLayout);
                View subViw = viewStub.inflate();
            }
            // @hl 新增数据为空的情况下也可以点击重试
    ...
    public View showRetry() {
        if (mRetryView == null) {
            ...
            if (-1 != mSkeletonLayout) {
                // @hl 新增占位布局
                ViewStub viewStub = mRetryView.findViewById(R.id.br_stateview_bgVs);
                viewStub.setLayoutResource(mSkeletonLayout);
                View subViw = viewStub.inflate();
            }
    ...
    public View showLoading() {
        if (mLoadingView == null) {
            ...
            if (-1 != mSkeletonLayout) {
                // @hl 新增占位布局
                ViewStub viewStub = mLoadingView.findViewById(R.id.bl_stateview_bgVs);
                viewStub.setLayoutResource(mSkeletonLayout);
                View subViw = viewStub.inflate();
            }
        }
